const calculadora = {
  ...
  valorCobranca: () => {
    ...
    const todasCobrancas = DATA.diarias.map(( diaria ) => diaria.cobranca.map(( cobranca ) => cobranca.valor));
    
    const somaDasCobrancas = todasCobrancas.length != 1 ? todasCobrancas.reduce(( array, i ) => array.concat(i)).reduce(( valor, i ) => valor+i) : null;
  ...
  valorTotal: () => {
    ...
    const cobranca = calculadora.valorCobranca();
    let horas = 0;
    let horas_extra = 0;
    let total_extra = 0;
    let total = 0;
