const minuta = {
  ...
  load: {
    ...
    descricao: () => {
      ...
      $('#ct_resultados').html(`
        ${DATA.tipo == 'corrida' || DATA.tipo == 'tabela' ? `${resultado.horas} Horas` : ``} <br>

        ${DATA.tipo == 'tabela' ? ` ${resultado.horas_extra} Horas extra <br> ${dinheiro(resultado.total_extra)} Total extra`: ``} <br>

        ${`${dinheiro(resultado.cobranca)} Cobrança`} <br>
